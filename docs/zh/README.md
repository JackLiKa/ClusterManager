...
Cluster Manager 将 `PSEUDO`（伪集群）与 `REAL`（真实集群）两种模式统一在一个 Web 界面中：你可以查看拓扑、启停节点、流式查看日志与监控、模拟消息收发，并登记真实服务与虚拟 TAP 节点进行混合联调。

![Cluster Manager 截图](../images/screenshot.png)
...
Maven 会自动编译 Vue 前端并复制到 `target/classes/static`，因此完整 UI 直接通过后端端口访问。
...
Vite 开发服务器运行在 `http://localhost:5173`，并将 `/api`、`/ws`、`/guide` 代理到 `http://localhost:8080`。
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `server.port` | `8080` | 后端与 SPA 的 HTTP 端口。 |
...
Cluster Manager 基于 [Apache License 2.0](../../LICENSE) 开源。
